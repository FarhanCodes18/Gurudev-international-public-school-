...
# I will find everything from <!-- Transport Modal --> to </body> and replace it
match = re.search(r'<!-- Transport Modal -->.*</body>', content, re.DOTALL)
if match:
    new_modal_and_script = """<!-- Transport Modal -->
...
<script type="module">
  ...
  window.closeTransportModal = function() {
    ...
    content = content[:match.start()] + new_modal_and_script
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("Force replaced student modal with correct CSS!")
else:
